...
export class ThirdStockAndSalesUtilityUpdateComponent implements OnInit {
  private _third: IThirdStockAndSalesUtility;
  isSaving: boolean;
  ...
  private subscribeToSaveResponse(result: Observable<HttpResponse<IThirdStockAndSalesUtility>>) {
    result.subscribe(
      (res: HttpResponse<IThirdStockAndSalesUtility>) => this.onSaveSuccess(res.body),
      (res: HttpErrorResponse) => this.onSaveError()
    );
  }

  private onSaveSuccess(result: IThirdStockAndSalesUtility) {
    this.isSaving = false;
    this.previousState();
  }
  ...
  set third(third: IThirdStockAndSalesUtility) {
    this._third = third;
  }
}
