...
export class AddressStockAndSalesUtilityUpdateComponent implements OnInit {
  private _address: IAddressStockAndSalesUtility;
  isSaving: boolean;
  ...
  private subscribeToSaveResponse(result: Observable<HttpResponse<IAddressStockAndSalesUtility>>) {
    result.subscribe(
      (res: HttpResponse<IAddressStockAndSalesUtility>) => this.onSaveSuccess(res.body),
      (res: HttpErrorResponse) => this.onSaveError()
    );
  }

  private onSaveSuccess(result: IAddressStockAndSalesUtility) {
    this.isSaving = false;
    this.previousState();
  }
  ...
  set address(address: IAddressStockAndSalesUtility) {
